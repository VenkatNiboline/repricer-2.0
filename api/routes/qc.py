...
from supabase_store import get_client, is_configured, list_qc_findings, resolve_qc_finding
...
def get_findings(
    ...
    if not is_configured():
        return []
    return list_qc_findings(resolved=resolved, severity=severity, limit=limit)
...
def verify_pending(user_id: Optional[str] = Depends(optional_user_id)):
    _ = user_id
    return verify_pending_reflections()


@router.post("/history/{history_id}/verify")
def verify_one(history_id: int, user_id: Optional[str] = Depends(optional_user_id)):
    _ = user_id
    if not is_configured():
        raise HTTPException(503, "Supabase not configured")
    client = get_client()
